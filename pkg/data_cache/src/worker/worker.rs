...
impl DataLoader {
    ...
    pub(crate) async fn load_data(&self, ctx: &SessionContext, table: &str, start_index: u64) -> datafusion::common::Result<()> {
        let memtable = IndexableMemTable::load(self.data_source.clone(), None, &ctx.state(), start_index).await.map_err(|error: DataFusionError| {
            error!("Error loading table: {:?}", error);
            error
        })?;
        ctx.register_table(table, Arc::new(memtable)).map_err(|e| DataFusionError::Execution(format!("Failed to register table: {}", e)))?;
